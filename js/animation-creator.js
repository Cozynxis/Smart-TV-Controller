(() => {
  ...
  const ed={topology:{left:0,top:0,right:0},leds:[],frames:[],selected:0,duration:5,fps:12,easing:'smoothstep',loop:true,pingpong:false,title:'Mijn animatie',emoji:'✨',description:'',color:'#168cff',brightness:100,advanced:false,previewing:false,previewId:null,previewStart:0};
  ...
  async function loadTopology(){
    const status=$('#creatorConnection');
    try{
      if(!window.state?.current){status.textContent='Geen TV';status.className='badge offline';return}
      const t=await api(`/api/ambilight/topology?deviceId=${encodeURIComponent(state.current.id)}`);
  ...
  function toggleLed(id,forceErase=false){const f=current();if(!f)return;const old=(f.pixels[id]||'#000000').toLowerCase(),paint=colorWithBrightness();f.pixels[id]=(forceErase||old!=='#000000')?'#000000':paint;render()}
  function render(){
    const st=$('#simpleCreatorStage');if(!st)return;st.querySelectorAll('.creator-led').forEach(x=>x.remove());const f=current();
  ...
  function addFrame(){const cur=current(),nextTime=Math.min(ed.duration,Math.max(cur.time+.5,(ed.selected+1)*(ed.duration/Math.max(4,ed.frames.length+1))));ed.frames.push(makeFrame(nextTime,cur.pixels));ed.frames.sort((a,b)=>a.time-b.time);ed.selected=ed.frames.findIndex(f=>f.time===nextTime);render();setStatus('Nieuw moment toegevoegd. Verplaats nu je lichtpunt.')}
  function eraseAll(){current().pixels=black();render()}
  ...
  async function send(){
    if(!window.state?.current)return setStatus('Verbind eerst je TV.','warning');
    if(ed.frames.length<2)return setStatus('Maak minimaal 2 momenten.','warning');
    stopPreview();window.__customAmbilightPlaying=true;
    try{await api('/api/ambilight/animation/stop',{method:'POST',body:JSON.stringify({deviceId:state.current.id})}).catch(()=>{});const r=await api('/api/ambilight/custom/start',{method:'POST',body:JSON.stringify({deviceId:state.current.id,animation:serialize()})});setStatus(`${r.emoji||'✨'} ${r.title||ed.title} speelt nu op de TV • ${r.fps} FPS`,'success')}
    catch(e){window.__customAmbilightPlaying=false;setStatus('Kon niet afspelen: '+e.message,'warning')}
  }
  async function stopTv(){if(!window.state?.current)return;try{await api('/api/ambilight/custom/stop',{method:'POST',body:JSON.stringify({deviceId:state.current.id})});window.__customAmbilightPlaying=false;setStatus('Animatie gestopt.','subtle')}catch(e){setStatus('Stoppen mislukt: '+e.message,'warning')}}
