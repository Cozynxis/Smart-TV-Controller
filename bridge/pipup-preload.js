...
express.application.listen=function(...args){if(!this.__pipupRoutes){this.__pipupRoutes=true;
 ...
 this.post('/api/announcements/send',async(req,res)=>{try{const ip=resolveTv(req.body),message=String(req.body?.message||'').trim();if(!message)throw new Error('Typ eerst een announcement.');const payload={duration:clamp(req.body?.duration,1,300,5),position:clamp(req.body?.position,0,4,0),title:String(req.body?.title||'').slice(0,80),titleColor:color(req.body?.titleColor,'#FFFFFF'),titleSize:clamp(req.body?.titleSize,8,60,20),message,messageColor:color(req.body?.messageColor,'#FFFFFF'),messageSize:clamp(req.body?.messageSize,8,60,14),backgroundColor:color(req.body?.backgroundColor,'#CC000000')};const r=await postJson(ip,'/notify',payload);if(r.status<200||r.status>=300)throw new Error(`PiPup HTTP ${r.status}${r.text?`: ${r.text.slice(0,100)}`:''}`);res.json({ok:true,status:r.status,ip,payload})}catch(e){res.status(502).json({error:e.message,code:'pipup_send_failed'})}});
 }
 return oldListen.apply(this,args)};
console.log('[ANNOUNCEMENTS] PiPup styling bridge v2 ready on port 7979');